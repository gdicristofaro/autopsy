    ...
    void makeArtifact(List<XRYKeyValuePair> keyValuePairs, Content parent, SleuthkitCase currentCase) throws TskCoreException, BlackboardException {
        ...
        if(!attributes.isEmpty()) {
            if (parent instanceof AbstractFile) {
                ((AbstractFile) parent).newDataArtifact(new BlackboardArtifact.Type(BlackboardArtifact.Type.TSK_WEB_BOOKMARK), attributes);
            } else {
                parent.newDataArtifact(new BlackboardArtifact.Type(BlackboardArtifact.Type.TSK_WEB_BOOKMARK), attributes, null);
            }
        }
    }
}