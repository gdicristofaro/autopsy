...
public class SolrSearchService implements KeywordSearchService, AutopsyService {
    ...
    void handleNewArtifacts(Blackboard.ArtifactsPostedEvent event) {
        for (BlackboardArtifact artifact : event.getArtifacts()) {
            if ((artifact.getArtifactTypeID() != BlackboardArtifact.Type.TSK_KEYWORD_HIT.getTypeID()) && // don't index KWH bc it's based on existing indexed text
                    (artifact.getArtifactTypeID() != BlackboardArtifact.Type.TSK_ASSOCIATED_OBJECT.getTypeID())){ //don't index AO bc it has only an artifact ID - no useful text 
                try {
                    index(artifact);
