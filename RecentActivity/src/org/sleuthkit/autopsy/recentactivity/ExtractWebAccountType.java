...
class ExtractWebAccountType extends Extract {
    ...
    private void extractDomainRoles(RoleProcessor roleProcessor, Content dataSource, IngestJobContext context) {
        try {
    ...
    private void extractServiceAccountRoles(RoleProcessor roleProcessor, Content dataSource, IngestJobContext context) {
        try {
            // Get web history blackboard artifacts
            Collection<BlackboardArtifact> listArtifacts = currentCase.getSleuthkitCase().getBlackboard().getArtifacts(
                    ARTIFACT_TYPE.TSK_WEB_ACCOUNT_TYPE.getTypeID(), dataSource.getId());
            ...
            for (BlackboardArtifact artifact : listArtifacts) {
                ...
                roleProcessor.addRole(domain, platformName, role, url, artifact);
            }

        } catch (TskCoreException e) {
            logger.log(Level.SEVERE, "Encountered error retrieving artifacts for service account role analysis", e); //NON-NLS
        }
    }
    ...
    void process(Content dataSource, IngestJobContext context, DataSourceIngestModuleProgress progressBar) {
        ...
        // extract roles from history artifacts
        extractDomainRoles(roleProcessor, dataSource, context);

        if (context.dataSourceIngestIsCancelled()) {
            return;
        }

        // extract roles from tsk_service_account artifacts
        extractServiceAccountRoles(roleProcessor, dataSource, context);
